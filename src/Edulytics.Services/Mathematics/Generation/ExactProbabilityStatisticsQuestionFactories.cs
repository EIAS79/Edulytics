...
public sealed class ExactSimpleProbabilityQuestionFactory
{
    ...
    public VerifiedGeneratedMathematicsProblem Generate(int variantKey, int difficultyBand)
    {
        ...
        var problem = new FunctionCallNode("probability_favourable_over_total_exact", [new IntegerNode(new BigInteger(favourable)), new IntegerNode(new BigInteger(total))]);
        return ExactProbabilityStatisticsGeneration.Build(FamilyId, Skill, problem, solver, verifier,
...
public sealed class ExactArithmeticMeanQuestionFactory
{
    ...
    public VerifiedGeneratedMathematicsProblem Generate(int variantKey, int difficultyBand)
    {
        ...
        for (var i = 0; i < count; i++) values[i] = ExactProbabilityStatisticsGeneration.Scalar(ref state, difficultyBand);
...
public sealed class ExactFrequencyMeanQuestionFactory
{
    ...
    public VerifiedGeneratedMathematicsProblem Generate(int variantKey, int difficultyBand)
    {
        ...
        {
            var value = ExactProbabilityStatisticsGeneration.Scalar(ref state, difficultyBand);
