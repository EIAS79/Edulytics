...
public sealed class LearningOutcomeConfiguration : IEntityTypeConfiguration<LearningOutcome>
{
    public void Configure(EntityTypeBuilder<LearningOutcome> b)
    {
        ...
        b.Property(x => x.Weight).HasPrecision(6, 3).IsRequired();
        b.Property(x => x.Order).IsRequired();

        b.HasIndex(x => new { x.SchoolId, x.CurriculumAdoptionId, x.Code })
            .IsUnique()
            .HasDatabaseName("UX_LearningOutcome_Adoption_Code")
        ...
        b.HasIndex(x => new
            ...
            .IsUnique()
            .HasDatabaseName("UX_LearningOutcome_Legacy_Code")
            .HasFilter("\"CurriculumAdoptionId\" IS NULL");

        b.HasIndex(x => new { x.SchoolId, x.TopicId, x.OfficialContentNodeId })
            .IsUnique();
        b.HasIndex(x => new { x.SchoolId, x.TopicId, x.Order })
            .IsUnique();
