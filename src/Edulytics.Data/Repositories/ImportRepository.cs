...
public sealed class ImportRepository : IImportRepository
{
    ...
    public async Task<ImportDataSnapshot> GetSnapshotAsync(
        ...
        new()
        {
            AcademicYears = await _db.AcademicYears.AsNoTracking().Where(x => x.SchoolId == schoolId).ToArrayAsync(cancellationToken),
            GradeLevels = await _db.GradeLevels.AsNoTracking().Where(x => x.SchoolId == schoolId).ToArrayAsync(cancellationToken),
            ClassGroups = await _db.ClassGroups.AsNoTracking().Where(x => x.SchoolId == schoolId).ToArrayAsync(cancellationToken),
            Subjects = await _db.Subjects.AsNoTracking().Where(x => x.SchoolId == schoolId).ToArrayAsync(cancellationToken),
            StudentProfiles = await _db.StudentProfiles.AsNoTracking().Where(x => x.SchoolId == schoolId).ToArrayAsync(cancellationToken),
            ...
            OutcomeMappings = await _db.QuestionLearningOutcomes.AsNoTracking().Where(x => x.SchoolId == schoolId).ToArrayAsync(cancellationToken),
            AssessmentResults = await _db.AssessmentResults.AsNoTracking().Where(x => x.SchoolId == schoolId).ToArrayAsync(cancellationToken)
        };
    ...
    public async Task<IReadOnlyList<ImportValidationError>> GetErrorsAsync(
        Guid schoolId,
        Guid batchId,
        CancellationToken cancellationToken = default) =>
    ...
    public async Task<ImportPersistenceResult> MarkValidationFailedAsync(
        Guid schoolId,
        Guid batchId,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _db.Entry(batch).Property(x => x.RowVersion).OriginalValue = expectedRowVersion;

            var oldErrors = await _db.ImportValidationErrors
                .Where(x => x.SchoolId == schoolId && x.ImportBatchId == batchId)
                .ToArrayAsync(cancellationToken);

            if (oldErrors.Length > 0)
                _db.ImportValidationErrors.RemoveRange(oldErrors);
            if (errors.Count > 0)
                _db.ImportValidationErrors.AddRange(errors);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _db.Entry(batch).Property(x => x.RowVersion).OriginalValue = expectedBatchRowVersion;

            var incomingActiveSeats = plan.Students.Count(
                x => !x.IsArchived && x.Status == AcademicStructureStatus.Active);
            ...
            if (incomingActiveSeats > 0)
            {
                ...
                {
                    subscription = await _db.SchoolSubscriptions
...
FOR UPDATE")
                ...
                if (subscription is not null)
                {
                    ...
                    var currentActiveSeats = await _db.StudentProfiles
                        .AsNoTracking()
                        .CountAsync(
                            x =>
                                x.SchoolId == schoolId &&
                    ...
                    if (currentActiveSeats + incomingActiveSeats > subscription.CommittedSeats)
                    {
                        await RollbackOwnedAsync(ownedTransaction, cancellationToken);
                        _db.ChangeTracker.Clear();
                        return ImportPersistenceResult.Failure(ImportPersistenceError.SeatLimit);
                    }
                }
            }

            foreach (var guard in plan.AcademicYearGuards.DistinctBy(x => x.Id))
            {
                var entity = await _db.AcademicYears.FirstOrDefaultAsync(
                    x => x.SchoolId == schoolId && x.Id == guard.Id,
                    cancellationToken);

                if (entity is null ||
                    entity.Status != AcademicStructureStatus.Active ||
                    !entity.RowVersion.SequenceEqual(guard.RowVersion))
                {
                    await RollbackOwnedAsync(ownedTransaction, cancellationToken);
                    return ImportPersistenceResult.Failure(ImportPersistenceError.Concurrency);
                }
            ...
            foreach (var guard in plan.ClassGroupGuards.DistinctBy(x => x.Id))
            {
                var entity = await _db.ClassGroups.FirstOrDefaultAsync(
                    x => x.SchoolId == schoolId && x.Id == guard.Id,
                    cancellationToken);
            ...
            foreach (var guard in plan.SubjectGuards.DistinctBy(x => x.Id))
            {
                var entity = await _db.Subjects.FirstOrDefaultAsync(
                    x => x.SchoolId == schoolId && x.Id == guard.Id,
                    cancellationToken);
            ...
            var trackedAssessments = new Dictionary<Guid, Assessment>();
            foreach (var guard in plan.AssessmentGuards.DistinctBy(x => x.Id))
            {
                var entity = await _db.Assessments.FirstOrDefaultAsync(
                    x => x.SchoolId == schoolId && x.Id == guard.Id,
                    cancellationToken);
            ...
            {
                if (!trackedAssessments.TryGetValue(result.AssessmentId, out var assessment))
                {
                    await RollbackOwnedAsync(ownedTransaction, cancellationToken);
                    return ImportPersistenceResult.Failure(ImportPersistenceError.InvalidState);
                }

                var validStudent = await _db.StudentProfiles.AnyAsync(
                    x =>
                        x.SchoolId == schoolId &&
                        x.Id == result.StudentProfileId &&
                        !x.IsArchived &&
                        x.Status == AcademicStructureStatus.Active,
                    cancellationToken);
                ...
                var enrolled = await _db.StudentEnrollments.AnyAsync(
                    x =>
                        x.SchoolId == schoolId &&
                        x.StudentProfileId == result.StudentProfileId &&
                        x.AcademicYearId == assessment.AcademicYearId &&
                        x.ClassGroupId == assessment.ClassGroupId,
                    cancellationToken);

                var alreadyExists = await _db.AssessmentResults.AnyAsync(
                    x =>
                        x.SchoolId == schoolId &&
                        x.AssessmentId == result.AssessmentId &&
                        x.StudentProfileId == result.StudentProfileId,
                    cancellationToken);

                if (!validStudent || !enrolled || alreadyExists)
                {
                    await RollbackOwnedAsync(ownedTransaction, cancellationToken);
                    return ImportPersistenceResult.Failure(ImportPersistenceError.InvalidState);
                }
            ...
            if (plan.TeacherAssignments.Count > 0)
                _db.TeacherAssignments.AddRange(plan.TeacherAssignments);
            if (plan.AssessmentResults.Count > 0)
                _db.AssessmentResults.AddRange(plan.AssessmentResults);
            if (plan.StudentAnswers.Count > 0)
                _db.StudentAnswers.AddRange(plan.StudentAnswers);
            if (plan.CurriculumMappings.Count > 0)
                _db.QuestionLearningOutcomes.AddRange(plan.CurriculumMappings);
            if (plan.OutboxMessages.Count > 0)
                _db.OutboxMessages.AddRange(plan.OutboxMessages);

            foreach (var assessmentId in plan.AssessmentsToTouch.Distinct())
            {
                if (trackedAssessments.TryGetValue(assessmentId, out var assessment))
                    assessment.UpdatedAtUtc = completedAtUtc;
            }

            batch.Status = ImportBatchStatus.Completed;
            batch.CompletedByUserId = actorUserId;
        ...
        catch
        {
            await RollbackOwnedAsync(ownedTransaction, cancellationToken);
            _db.ChangeTracker.Clear();
            return ImportPersistenceResult.Failure(ImportPersistenceError.Unknown);
        }
        finally
        {
