...
internal static class Phase38CleanBaselineV2
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                Write(
                    "PHASE38_CLEANUP_BEGIN operational_tables={0} preserved_tables={1} strategy=delete_fk_order",
                ...
                await ExecuteAsync(
                    ...
                    "(\"CompletedAtUtc\", \"SuperAdminId\", \"OperationalTablesCleared\", \"Strategy\") " +
                    "VALUES (NOW(), @id::uuid, @count, 'delete_fk_order')",
                    cancellationToken,
                    nativeTransaction,
                    "id",
                    superAdminId,
                ...
                Write(
                    "PHASE38_CLEANUP_COMMITTED operational_tables={0} real_superadmin_preserved=1 strategy=delete_fk_order",
                    operationalTables.Length);
            }
            catch
            {
        ...
        DbTransaction? transaction = null)
    {
        foreach (var table in PreservedTables.OrderBy(x => x, StringComparer.Ordinal))
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Phase38 preserved-table content verification failed for '{table}'.");
            }
        }
    }
