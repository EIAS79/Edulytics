...
public static class PracticeMathVisualRenderer
{
    ...
    private static string CoordinateGradient(JsonElement p)
    {
        ...
        {
            var px = X(i).ToString("0.##", CultureInfo.InvariantCulture);
            var py = Y(i).ToString("0.##", CultureInfo.InvariantCulture);
    ...
    private static string ParallelLines(JsonElement p)
    {
        ...
        var expected = GetInt(p, "expectedAngle");
        ...
        Text(sb, 210, 248, $"Find x = {expected}° only after reasoning.", "hint");
        return SvgEnd(sb);
    }
    ...
    private static string Similarity(JsonElement p)
    {
        ...
        var target = p.TryGetProperty("targetLength", out _) ? GetInt(p, "targetLength") : 0;
        var factor = p.TryGetProperty("scaleFactor", out _) ? GetInt(p, "scaleFactor") : (source == 0 ? 0 : target / source);
        ...
        Text(sb, 290, 242, target > 0 ? target.ToString(CultureInfo.InvariantCulture) : "?", "label");
        Text(sb, 94, 90, "A", "point-label");
        Text(sb, 301, 40, "A′", "point-label");
        Text(sb, 210, 28, $"scale factor {factor}", "hint");
        return SvgEnd(sb);
    }
    ...
    private static string Congruence(JsonElement p)
    {
        ...
        var sb = SvgStart("Two triangles with corresponding equality marks for a congruence question.");
    ...
    private static string RightTriangle(JsonElement p)
    {
        ...
        Text(sb, 45, 140, vertical.HasValue ? vertical.Value.ToString(CultureInfo.InvariantCulture) : "opposite", "label");
        Text(sb, 205, 238, horizontal.HasValue ? horizontal.Value.ToString(CultureInfo.InvariantCulture) : "adjacent", "label");
        if (hypotenuse.HasValue)
            Text(sb, 225, 122, hypotenuse.Value.ToString(CultureInfo.InvariantCulture), "label");
        else
            Text(sb, 225, 122, "hypotenuse", "hint");
